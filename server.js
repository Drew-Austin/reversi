	...
	function log(){
	 ...
	 socket.on('join_room', function(payload){
		...
		for(var socket_in_room in roomObject.sockets){
           ...
           if(room !== 'lobby'){
            send_game_update(socket,room,'intail update');
        }
...
/* game_start command */
 ...
 socket.on('play_token', function(payload){
    ...
    if(('undefined' === typeof player) || !player){
       var error_message = 'server  can\'t ,identify who you are (try going back one screen)';
       log(error_message);
       socket.emit('play_token_response ', {
          Result: 'fail',
          message: error_message
...
/* helper function checks the color on the c by adding stuff to c */
...
function send_game_update(socket, game_id, message){
    /* check to see if game with game id exist */
